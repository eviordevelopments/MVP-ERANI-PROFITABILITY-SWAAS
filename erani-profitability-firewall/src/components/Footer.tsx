...
export default function Footer() {
    return (
        <footer className="relative bg-erani-black pt-24 pb-12 px-6 border-t border-white/5 overflow-hidden">
            {/* Background Static/Motion lines */}
            <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-erani-blue/20 to-transparent" />
            ...
            <div className="container max-w-7xl mx-auto relative z-10">
                        ...
                        <div className="flex gap-4">
                            {[Linkedin, Twitter, Globe].map((Icon, i) => (
                                <a
                                    key={i}
                                    href="#"
                                    className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-erani-blue hover:border-erani-blue/50 transition-all group"
                                >
                            ...
                            <ul className="space-y-4">
                                {column.links.map((link, j) => (
                                    <li key={j}>
                                        <a
                                            href={link.href}
                                            className="text-gray-500 hover:text-erani-blue transition-colors inline-flex items-center gap-1 group font-medium"
                                        >
                                            {link.name}
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        >
                            <div className="w-2 h-2 rounded-full bg-erani-blue border border-erani-blue/50 shadow-[0_0_10px_rgba(0,85,160,0.5)]" />
                            <span className="text-[10px] text-gray-500 font-medium uppercase tracking-widest">System Status: Optimal</span>
                        </motion.div>

                        <div className="text-[10px] text-gray-500 font-medium uppercase tracking-widest flex items-center gap-1">
            ...
            <div className="absolute bottom-0 right-0 w-1/3 h-[500px] bg-erani-blue/5 blur-[120px] rounded-full translate-y-1/2 translate-x-1/2" />

            {/* FULL BRAND GRADIENT AT THE VERY BOTTOM */}
            <div className="absolute bottom-0 left-0 right-0 h-4 bg-gradient-to-t from-erani-blue/40 to-transparent" />
            <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-erani-blue/60 shadow-[0_0_15px_rgba(0,85,160,0.8)]" />
        </footer>
    );
}
